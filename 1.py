...
class Attendance:
    ...
    def __init__(self, root):
        ...
        header = Label(root, text="ATTENDANCE MANAGEMENT SYSTEM",
                       bg="white", fg="black", font=("verdana", 36, "bold"))
        ...
        left_frame = LabelFrame(main_frame, bd=2, bg='white', relief=RIDGE,
                                fg="black", text="Details", font=("comicsansns", 12, "italic"))
        ...
        attendaceID_lable = Label(left_inside_frame, text="Enrollment No.", font=(
            "comicsansns", 14, "italic"), fg="black", bg="white")

        attendaceID_lable.grid(row=0, column=0, pady=5, sticky=W)

        attendaceID_entry = ttk.Entry(
            left_inside_frame, width=16, textvariable=self.var_atten_id, font=("comicsansns", 12, "italic"))

        attendaceID_entry.grid(row=0, column=1, pady=5, sticky=W)

        rollLable = Label(left_inside_frame, text="RollNo", font=(
            "comicsansns", 14, "italic"), fg="black", bg="white")

        rollLable.grid(row=0, column=2, pady=8, sticky=W)

        atten_roll = ttk.Entry(left_inside_frame, width=16, textvariable=self.var_atten_roll, font=(
            "comicsansns", 12, "italic"))

        atten_roll.grid(row=0, column=3, pady=8, sticky=W)

        nameLable = Label(left_inside_frame, text="Name", font=(
            "comicsansns", 14, "italic"), fg="black", bg="white")

        nameLable.grid(row=1, column=2, pady=8, sticky=W)

        atten_roll = ttk.Entry(left_inside_frame, width=16, textvariable=self.var_atten_name, font=(
            "comicsansns", 12, "italic"))

        atten_roll.grid(row=1, column=3, pady=8, sticky=W)

        depLable = Label(left_inside_frame, text="Department", font=(
            "comicsansns", 14, "italic"), fg="black", bg="white")

        depLable.grid(row=1, column=0, pady=8, sticky=W)

        atten_dep = ttk.Entry(left_inside_frame, width=16, textvariable=self.var_atten_dep, font=(
            "comicsansns", 12, "italic"))

        atten_dep.grid(row=1, column=1, pady=8, sticky=W)

        subjectLable = Label(left_inside_frame, text="Subject", font=(
            "comicsansns", 14, "italic"), fg="black", bg="white")

        subjectLable.grid(row=2, column=0, pady=8, sticky=W)

        atten_subject = ttk.Entry(left_inside_frame, width=16, textvariable=self.var_atten_subject, font=(
            "comicsansns", 12, "italic"))

        atten_subject.grid(row=2, column=1, pady=8, sticky=W)

        dateLable = Label(left_inside_frame, text="Date", font=(
            "comicsansns", 14, "italic"), fg="black", bg="white")

        dateLable.grid(row=2, column=2, pady=8, sticky=W)

        atten_date = ttk.Entry(left_inside_frame, width=16, textvariable=self.var_atten_date, font=(
            "comicsansns", 12, "italic"))

        atten_date.grid(row=2, column=3, pady=8, sticky=W)

        attendanceLable = Label(left_inside_frame, text="Status", font=(
            "comicsansns", 14, "italic"), fg="black", bg="white")

        attendanceLable.grid(row=6, column=0, pady=8, sticky=W)

        self.atten_status = ttk.Combobox(left_inside_frame, width=14, textvariable=self.var_atten_attendance, font=(
            "comicsansns", 12, "italic"), state="readonly")
        ...
        save_btn = Button(btn_frame, text="Save", command=self.add_students, width=13, font=(
            "comicsansns", 11, "italic"), bg="black", fg="white")

        save_btn.grid(row=0, column=0, padx=5)

        update_btn = Button(btn_frame, text="Update", command=self.update_student, width=13, font=(
            "comicsansns", 11, "italic"), bg="black", fg="white")

        update_btn.grid(row=0, column=1, padx=5)

        delete_btn = Button(btn_frame, text="Delete",command=self.delete_data, width=13, font=(
            "comicsansns", 11, "italic"), bg="black", fg="white")

        delete_btn.grid(row=0, column=2, padx=5)

        reset_btn = Button(btn_frame, text="Reset", command=self.reset_data, width=13, font=(
            "comicsansns", 11, "italic"), bg="black", fg="white")

        reset_btn.grid(row=0, column=3, padx=8)

        right_frame = LabelFrame(main_frame, bd=2, bg='white', fg="black", relief=RIDGE,
                                 text="Attendance Details", font=("comicsansns", 12, "italic"))
        ...
        self.AttendanceReportTable = ttk.Treeview(table_frame, columns=(
            "id", "roll", "name", "department", "subject", "date", "attendance"), xscrollcommand=scroll_x.set, yscrollcommand=scroll_y.set)
    ...
    def add_students(self):
        ...
        if self.var_atten_roll.get() == "":
            ...
            con = mysql.connector.connect(
                host="localhost", port=3306, user="root", passwd="", database="attendance")
    ...
    def update_student(self):
        ...
        if self.var_atten_roll.get() == "":
            ...
            con = pymysql.connect(
                host="localhost", port=3306, user="root", password="", database="attendance")

            cur = con.cursor()

            cur.execute("update students set id=%s,name=%s,department=%s,subject=%s,date=%s,attendance=%s where roll=%s", (self.var_atten_id.get(),
                                                                                                                           self.var_atten_name.get(),
                                                                                                                           self.var_atten_dep.get(),
                                                                                                                            self.var_atten_subject.get(),
                                                                                                                           self.var_atten_date.get(),
    ...
    def delete_data(self):
        ...
        if id == "":
            messagebox.showerror("Error", "Attendance ID must be required.")
        else:
            try:
